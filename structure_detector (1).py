...
def ask_gpt4o_for_transition_point_fast(segments, beats_str, title, artist, duration):
    """
    Ask GPT to detect BOTH chorus_start and chorus_end times.
    Returns transition points for proper DJ beatmatch transitions.
    """
    ...
    for seg in segments[:25]:  # First 25 segments (~first 2.5 minutes)
...
TASK: Find the FIRST CHORUS of this song for DJ mixing.

The chorus is the catchy, repeated hook part that comes after the intro and first verse. 
It's usually the most memorable part with repeated lyrics.

I need BOTH the START and END of the first chorus for a DJ transition:
- At chorus START: the next song will begin playing (overlapping)
- At chorus END: this song fades out completely

STEP 1: Identify which lines are the FIRST CHORUS
STEP 2: Find the FIRST LINE of that chorus (when it starts)
STEP 3: Find the LAST LINE of that chorus (when it ends)
STEP 4: Give me BOTH timestamps

Return JSON:
{{
  "first_chorus_lines": ["line 1", "line 2", ...],
  "first_chorus_line": "<the opening line of the first chorus>",
  "last_chorus_line": "<the final line of the first chorus>",
  "chorus_start_time": <START timestamp when the first chorus BEGINS>,
  "chorus_end_time": <END timestamp when the first chorus ENDS>,
  ...
  "intro_duration_sec": <when does first vocal start>
    ...
    # Extract chorus start and end times
    chorus_start = float(result.get("chorus_start_time", 30.0))
    chorus_end = float(result.get("chorus_end_time", chorus_start + 20.0))
    
    # Validate: chorus_end must be after chorus_start
    if chorus_end <= chorus_start:
        chorus_end = chorus_start + 15.0  # Default 15 second chorus
    
    print(f"  → First chorus: {chorus_start:.1f}s - {chorus_end:.1f}s ({chorus_end - chorus_start:.1f}s duration)")
    print(f"  → First line: {result.get('first_chorus_line', 'unknown')}")
    print(f"  → Last line: {result.get('last_chorus_line', 'unknown')}")
    
    # Store both values
    result["chorus_start_time"] = chorus_start
    result["chorus_end_time"] = chorus_end
    result["transition_point"] = chorus_end  # For backward compatibility
...
def analyze_structure_fast(title, artist, filename, bpm, SONGS_DIR="./songs"):
    ...
    try:
        ...
        structure_data = {
            ...
            # NEW: Include chorus_start and chorus_end for proper DJ transitions
            "chorus_start_time": result.get("chorus_start_time", transition_point - 15.0),
            "chorus_end_time": result.get("chorus_end_time", transition_point),
            # Include all transition candidates and energy analysis
