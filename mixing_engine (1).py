...
# ================= TRANSITIONS =================
def apply_chorus_beatmatch(outgoing_track: AudioSegment, incoming_track: AudioSegment, 
                           chorus_start_ms: int, chorus_end_ms: int, overlap_duration_ms: int,
                           bpm_from: float, bpm_to: float, track_names: tuple = ("Outgoing", "Incoming")):
    """
    PROFESSIONAL DJ CHORUS BEATMATCH TRANSITION
    
    Both tracks play together during the transition with:
    - Beat-aligned mixing (downbeats match)
    - EQ filtering (low-pass outgoing, high-pass incoming)
    - Smooth crossfade
    
    Flow:
    1. Outgoing plays solo until chorus_start
    2. At chorus_start: incoming enters, both play together (EQ filtered)
    3. During overlap: gradual crossfade with EQ swap
    4. At chorus_end: outgoing fades out completely
    5. Incoming continues solo
    
    Returns: Combined audio with professional DJ transition
    """
    print(f"   🎧 CHORUS BEATMATCH: {track_names[0]} → {track_names[1]}")
    
    # Safety check: chorus_start must be before chorus_end
    if chorus_start_ms >= chorus_end_ms:
        print(f"      ⚠️ Invalid timing (start {chorus_start_ms/1000:.1f}s >= end {chorus_end_ms/1000:.1f}s), adjusting...")
        # Calculate proper timing: end at chorus_end, work backwards
        chorus_start_ms = max(0, chorus_end_ms - overlap_duration_ms)
    
    # Ensure we have valid durations
    if chorus_end_ms <= chorus_start_ms:
        chorus_end_ms = chorus_start_ms + overlap_duration_ms
    
    # Ensure overlap doesn't exceed actual range
    actual_overlap_ms = chorus_end_ms - chorus_start_ms
    if actual_overlap_ms < overlap_duration_ms:
        overlap_duration_ms = max(actual_overlap_ms, 5000)  # At least 5 seconds
    
    print(f"      Blend starts: {chorus_start_ms/1000:.1f}s")
    print(f"      Blend ends: {chorus_end_ms/1000:.1f}s")
    print(f"      Overlap: {overlap_duration_ms/1000:.1f}s")
    
    # ===== STEP 1: BPM MATCHING (Time-stretch incoming to match outgoing) =====
    if bpm_from > 0 and bpm_to > 0 and abs(bpm_from - bpm_to) > 0.5:
        stretch_factor = bpm_from / bpm_to  # Stretch incoming to match outgoing BPM
        stretch_factor = np.clip(stretch_factor, 0.92, 1.08)  # Allow up to 8% stretch
        if abs(stretch_factor - 1.0) > 0.005:
            print(f"      🎛️ BPM match: {bpm_to:.0f} → {bpm_from:.0f} (stretch {stretch_factor:.4f}x)")
            incoming_track = time_stretch_audio(incoming_track, stretch_factor)
    
    # ===== STEP 2: BEAT ALIGNMENT (Align downbeats) =====
    print(f"      🎯 Aligning beats...")
    try:
        # Get section around transition point for beat matching
        match_section = outgoing_track[max(0, chorus_start_ms - 2000):chorus_start_ms + 5000]
        incoming_section = incoming_track[:min(10000, len(incoming_track))]
        
        # Use perfect beat alignment
        aligned_incoming, shift_ms = align_beats_perfect(
            match_section, incoming_section, overlap_duration_ms,
            bpm_from, bpm_from, track_names=track_names
        )
        
        if len(aligned_incoming) > 0 and abs(shift_ms) > 10:
            # Apply the shift to full incoming track
            if shift_ms > 0:
                incoming_track = AudioSegment.silent(int(shift_ms)) + incoming_track
            else:
                trim_ms = min(abs(int(shift_ms)), len(incoming_track) - 1000)
                incoming_track = incoming_track[trim_ms:]
            print(f"      ✅ Beat shift: {shift_ms:.1f}ms")
    except Exception as e:
        print(f"      ⚠️ Beat alignment skipped: {e}")
    
    # ===== STEP 3: BUILD THE TRANSITION =====
    
    # Part 1: Outgoing plays SOLO until chorus_start (when incoming begins)
    before_blend = outgoing_track[:chorus_start_ms]
    print(f"      Solo outgoing: {len(before_blend)/1000:.1f}s")
    
    # Part 2: OVERLAP SECTION - BOTH tracks play together!
    # Outgoing continues from chorus_start to chorus_end (fading out)
    # Incoming starts from beginning and plays alongside
    
    # The overlap is from chorus_start to chorus_end
    blend_duration_ms = chorus_end_ms - chorus_start_ms
    blend_duration_ms = max(blend_duration_ms, 8000)  # At least 8 seconds blend
    
    # Make sure we don't exceed track lengths
    blend_duration_ms = min(blend_duration_ms, len(outgoing_track) - chorus_start_ms)
    blend_duration_ms = min(blend_duration_ms, len(incoming_track))
    
    # Outgoing: plays from chorus_start, continuing to play and fade out
    outgoing_overlap = outgoing_track[chorus_start_ms:chorus_start_ms + blend_duration_ms]
    # Incoming: starts from beginning, fades in while outgoing plays
    incoming_overlap = incoming_track[:blend_duration_ms]
    
    # Ensure same length
    min_len = min(len(outgoing_overlap), len(incoming_overlap))
    outgoing_overlap = outgoing_overlap[:min_len]
    incoming_overlap = incoming_overlap[:min_len]
    blend_duration_ms = min_len
    
    print(f"      🎵 OVERLAP: {blend_duration_ms/1000:.1f}s (BOTH songs playing together!)")
    print(f"         Outgoing segment: {len(outgoing_overlap)/1000:.1f}s")
    print(f"         Incoming segment: {len(incoming_overlap)/1000:.1f}s")
    
    # Safety check
    if len(outgoing_overlap) < 1000 or len(incoming_overlap) < 1000:
        print(f"      ⚠️ Cannot create overlap, using simple crossfade")
        return outgoing_track.append(incoming_track, crossfade=min(5000, len(outgoing_track)//4))
    
    # ===== TRUE DJ CROSSFADE: Both tracks play together =====
    # IMPORTANT: We need BOTH tracks to be clearly audible during the overlap
    # 
    # Strategy:
    # - Outgoing: Full volume for first half, then fade out in second half
    # - Incoming: Fade in during first half, full volume for second half
    # This ensures both tracks are loud and clear during the middle of the blend
    
    half_blend = blend_duration_ms // 2
    
    # === OUTGOING: Keep full volume, only fade at the end ===
    # First 2/3: full volume, Last 1/3: fade out
    fade_start = int(blend_duration_ms * 0.6)  # Start fading at 60%
    outgoing_part1 = outgoing_overlap[:fade_start]  # Full volume
    outgoing_part2 = outgoing_overlap[fade_start:].fade_out(len(outgoing_overlap) - fade_start)  # Fade out
    outgoing_processed = outgoing_part1 + outgoing_part2
    
    # === INCOMING: Fade in quickly, then full volume ===
    # First 1/3: fade in, Last 2/3: full volume
    fade_end = int(blend_duration_ms * 0.4)  # Finish fading in at 40%
    incoming_part1 = incoming_overlap[:fade_end].fade_in(fade_end)  # Fade in
    incoming_part2 = incoming_overlap[fade_end:]  # Full volume
    incoming_processed = incoming_part1 + incoming_part2
    
    # Ensure same length after processing
    min_processed_len = min(len(outgoing_processed), len(incoming_processed))
    outgoing_processed = outgoing_processed[:min_processed_len]
    incoming_processed = incoming_processed[:min_processed_len]
    
    # === OVERLAY: Combine both tracks ===
    # Lower the incoming slightly to prevent clipping when both play together
    incoming_processed = incoming_processed - 3  # -3dB
    
    overlap_mix = outgoing_processed.overlay(incoming_processed)
    
    print(f"      ✅ CROSSFADE BLEND:")
    print(f"         Outgoing: full volume until {fade_start/1000:.1f}s, then fades out")
    print(f"         Incoming: fades in until {fade_end/1000:.1f}s, then full volume")
    print(f"         Combined: {len(overlap_mix)/1000:.1f}s with both tracks")
    
    # Part 3: Incoming continues SOLO after the blend
    incoming_rest = incoming_track[blend_duration_ms:]
    print(f"      Solo incoming: {len(incoming_rest)/1000:.1f}s")
    
    # ===== STEP 4: COMBINE ALL PARTS =====
    # before_blend: Outgoing plays solo
    # overlap_mix: BOTH tracks play together (the DJ blend)
    # incoming_rest: Incoming plays solo
    result = before_blend + overlap_mix + incoming_rest
    
    print(f"   ✅ Beatmatch complete: {len(result)/1000:.1f}s total")
    print(f"      Structure: {len(before_blend)/1000:.1f}s solo → {len(overlap_mix)/1000:.1f}s blend → {len(incoming_rest)/1000:.1f}s solo")
...
def generate_mix(mixing_plan_json: str = "output/mixing_plan.json", 
    ...
    for idx, entry in enumerate(plan):
        ...
        transition_type = entry.get("transition_type", "chorus-beatmatch")
        compatibility_score = entry.get("compatibility_score", "N/A")
        ...
        print(f"\n{'='*60}")
        print(f"{idx+1}. {from_title} → {to_title}")
        print(f"   🎵 Transition: {transition_type.upper()} (Score: {compatibility_score})")
        print(f"   Mix length before: {len(mix)/1000:.1f}s")
        
        # === CHORUS BEATMATCH: Professional DJ blend ===
        if transition_type == "chorus-beatmatch":
            chorus_start_sec = safe_float(entry.get("chorus_start_sec"), 
                                          entry.get("transition_point", 45.0))
            chorus_end_sec = safe_float(entry.get("first_chorus_end_sec"), chorus_start_sec + 16.0)
            overlap_duration_sec = safe_float(entry.get("overlap_duration_sec"), 12.0)
            
            chorus_start_ms = ms(chorus_start_sec)
            chorus_end_ms = ms(chorus_end_sec)
            overlap_duration_ms = ms(overlap_duration_sec)
            
            print(f"   Blend start: {chorus_start_sec:.1f}s")
            print(f"   Blend end: {chorus_end_sec:.1f}s")
            print(f"   Overlap: {overlap_duration_sec:.1f}s")
            print(f"   BPM: {bpm_from:.0f} → {bpm_to:.0f}")
            
            if previous_track_audio:
                track_names = (from_title, to_title)
                transition_result = apply_chorus_beatmatch(
                    outgoing_track=previous_track_audio,
                    incoming_track=to_audio,
                    chorus_start_ms=chorus_start_ms,
                    chorus_end_ms=chorus_end_ms,
                ...
                # Replace previous track with transition result
                ...
                incoming_start_in_mix = previous_track_start + chorus_start_ms
                mix_overview_data.append({
                    'name': to_title,
        ...
        # === ECHO TRANSITION: Safe fallback for incompatible tracks ===
            ...
            print(f"   Chorus end: {first_chorus_end_sec:.1f}s")
            ...
            if previous_track_audio:
                ...
                incoming_start_in_mix = previous_track_start + first_chorus_end_ms + echo_duration_ms
                mix_overview_data.append({
                    'name': to_title,
                    'start_ms': incoming_start_in_mix,
                    'duration_ms': len(to_audio),
        ...
        # === FADE IN: First track only ===
        elif transition_type == "fade-in":
            mix = mix.append(to_audio.fade_in(2000), crossfade=0)
        
        # === FALLBACK: Simple crossfade ===
        else:
            print(f"   [INFO] Unknown transition type '{transition_type}', using crossfade")
            mix = mix.append(to_audio, crossfade=3000)
